...
type AutoCCTPSuite struct {
	// Chain with AutoCCTP module
...
func NewAutoCCTPSuite(t *testing.T, isZeroFees bool, isIBC bool) (context.Context, AutoCCTPSuite) {
	...
	chainSpecs := []*interchaintest.ChainSpec{
		createAutoCCTPChainSpec(ctx, &suite, isZeroFees),
	}
	if isIBC {
		chainSpecs = append(
			chainSpecs,
...
func createAutoCCTPChainSpec(ctx context.Context, suite *AutoCCTPSuite, isZeroFees bool) *interchaintest.ChainSpec {
	gasPrices := "1uusdc"
	if isZeroFees {
		gasPrices = "0uusdc"
	}
...
func preGenesis(ctx context.Context, suite *AutoCCTPSuite) func(ibc.Chain) error {
...
func modifyGenesis(suite *AutoCCTPSuite) func(cc ibc.ChainConfig, b []byte) ([]byte, error) {
